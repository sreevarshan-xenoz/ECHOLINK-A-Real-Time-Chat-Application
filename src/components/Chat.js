...
const MessageStatus = memo(({ status, timestamp }) => {
  ...
  return (
    <Flex alignItems='center' fontSize='xs' color='gray.500'>
      ...
      {timestamp && <Text>{new Date(timestamp).toLocaleTimeString()}</Text>}
    </Flex>
  );
});
...
const MessageItem = memo(({ message, onReaction, onRetry, onMarkAsRead, onDelete }) => {
  ...
  return (
    <Box alignSelf={isSent ? 'flex-end' : 'flex-start'} maxWidth='80%' mb={3}>
      <Card
        bg={
          isSent
            ? colorMode === 'dark'
...
const Chat = ({
  ...
  const useBasicChatbot = !isAiInitialized;
  ...
  const basicChatbotResponses = {
    hello:
      "Hello! I'm a basic chatbot. I can help you with simple tasks. Type 'help' to see what I can do.",
  ...
  const handleRetry = useCallback(
    (message) => {
      ...
      if (success) {
        setMessages((prev) =>
          prev.map((msg) => (msg.id === message.id ? { ...msg, status: 'sent', error: null } : msg))
        );
      } else {
        setMessages((prev) =>
          prev.map((msg) =>
  ...
  const filteredMessages = useMemo(() => {
    return messages.filter(
      (msg) =>
        !searchQuery || (msg.text && msg.text.toLowerCase().includes(searchQuery.toLowerCase()))
    );
          ...
          <div className='message-content'>
              ...
              <>
                ...
                {message.isEdited && (
                  <span className='edit-indicator'>
                    edited{' '}
                    {new Date(message.editHistory?.slice(-1)[0]?.timestamp).toLocaleTimeString()}
                  </span>
                )}
                {translatedMessages.has(message.id) && (
                  <p className='translated-text'>{translatedMessages.get(message.id)}</p>
                )}
                <MessageStatus status={message.status} timestamp={message.timestamp} />
                {message.status === 'error' && (
                  <div className='message-actions'>
                    <button className='retry-button' onClick={() => handleRetry(message)}>
                      Retry
                    </button>
                    <button className='delete-button' onClick={() => handleDeleteMessage(message)}>
                      Delete
                    </button>
                  </div>
                )}
                ...
                {message.language && <span className='language-indicator'>{message.language}</span>}
              </>
            )}
              ...
              <div className='voice-message'>
                ...
                {message.transcription && (
                  <div className='transcription'>
                    <p>
                      <i>Transcription: {message.transcription}</i>
                    </p>
                  </div>
                )}
              </div>
            ...
            {message.type === 'FILE_META' && (
              <div className='file-message'>
                ...
                <span className='file-size'>{(message.size / 1024).toFixed(1)} KB</span>
              </div>
            )}
            ...
            <div className='settings-group'>
              <label>
                <input
                  ...
                  onChange={(e) =>
                    handleSettingsChange('notifications', 'enabled', e.target.checked)
                  }
                />
            ...
            <div className='settings-group'>
              <label>
                <input
                  ...
                  onChange={(e) =>
                    handleSettingsChange('privacy', 'encryptMessages', e.target.checked)
                  }
                />
                Encrypt Messages
              </label>
              <label>
                <input
                  ...
                  onChange={(e) =>
                    handleSettingsChange('privacy', 'autoDeleteMessages', e.target.checked)
                  }
                />
          ...
          <Flex alignItems='center'>
            ...
            <Box>
              <Heading size='md'>
                {isAIChatActive
                  ? 'Echo AI Assistant'
                  : userProfile?.displayName || selectedPeer || 'Select a peer'}
              </Heading>
              <Flex alignItems='center'>
                <Badge
      ...
      <Flex flex='1' overflow='hidden'>
        ...
        >
          <VStack spacing={4} align='stretch'>
            ...
            >
              ...
              <Text fontSize='xs' mb={2}>
                2. Or enter their ID below to connect
              </Text>
              <Input
              ...
              />
              <Button colorScheme='blue' size='sm' width='full' borderRadius='md'>
                Connect
              </Button>
            </Box>
          ...
          {filteredMessages.length === 0 ? (
            ...
            >
              ...
              <Text mb={4} color={colorMode === 'dark' ? 'gray.400' : 'gray.600'} maxWidth='400px'>
                Connect with someone to chat or try out the AI assistant. Your messages will appear
                here.
              </Text>
              <HStack spacing={4}>
                <Button
        ...
        >
          <VStack spacing={4} align='stretch'>
            ...
            >
              <LazyCollaborativeWhiteboard
                onShareWhiteboard={(blob, filename) => {
      ...
      {connectionState !== 'connected' && (
        ...
        >
          {connectionState === 'reconnecting'
            ? 'Reconnecting...'
            : 'Connection error. Working offline.'}
        </Box>
      )}
